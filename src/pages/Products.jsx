...
export default function Products() {
  ...
  const unArchiveProduct = async (id) => {
    try {
      ...
      const res = await axios
        .patch(
          `${process.env.REACT_APP_API_HOST}/products/archive/${id}?is_active=true`
        )
  ...
  const archiveProduct = async (id) => {
    try {
      ...
      const res = await axios
        .patch(
          `${process.env.REACT_APP_API_HOST}/products/archive/${id}?is_active=false`
        )
        .then((res) => res.data.metadata);
      setLoading(false);
      toast.success(`${res.msg}`);
  ...
  const handleDeleteProduct = async (deletedId) => {
    try {
      setLoading(true);
      const res = await axios
        .delete(`${process.env.REACT_APP_API_HOST}/products/${deletedId}`)
        .then((res) => res.data.metadata);
      setLoading(false);
      toast.success(`${res.msg}`);
    } catch (error) {
  ...
  return (
    <>
      ...
      <div className="flex justify-end my-4">
        <Link
          to={`${process.env.REACT_APP_API_HOST}/products/summary`}
          className="btn btn-info mr-1  btn-outline normal-case btn-sm"
        >
          Download CSV
        </Link>
        <Link
          to={`${process.env.REACT_APP_API_HOST}/products/summaryexcel`}
          className="btn btn-secondary  mr-1  btn-outline normal-case btn-sm"
        >
          Download Excel
        </Link>
        <Link
      ...
      {search ? null : (
        <select
          className="select rounded select-bordered select-sm mt-4 w-full"
