  ...
  return (
    <>
      ...
      <div className="store-address">
        {" "}
        Jl. Puri Cendana No.112, Sumberjaya, Kec. Tambun Sel., Kabupaten Bekasi,
        Jawa Barat 17510
      </div>
      <div className="trx-info">
        ...
        <div className="trx-date">
          Date :{" "}
          {moment(orders?.orders?.createdAt).format("DD MMM YYYY, HH:mm")}
        </div>
        <div className="trx-cashier_name">
          Kasir : {orders?.order?.User?.name}
        </div>
      </div>
      ...
      <div className="price-container">
        <div className="price_total pay">
          <div>Harga Total : </div>
          <div>Rp{formatRupiah(orders?.order?.amount)}</div>
        </div>
        <div className="pay_amount pay">
          <div>Bayar : </div>
          <div>Rp{formatRupiah(orders?.order?.pay_amount)}</div>
        </div>
        <div className="pay_change pay">
          ...
          <div>
            Rp
            {formatRupiah(
              Number(orders?.order?.pay_amount) - Number(orders?.order?.amount)
            )}
          </div>
        </div>
      </div>
